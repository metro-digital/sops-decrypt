  ...
  "node_modules/@actions/tool-cache/lib/tool-cache.js"(exports) {
    ...
    function cacheDir(sourceDir, tool, version2, arch) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        arch = arch || os.arch();
        core5.debug(`Caching tool ${tool} ${version2} ${arch}`);
    ...
    function find2(toolName2, versionSpec, arch) {
      if (!toolName2) {
      ...
      if (!isExplicitVersion(versionSpec)) {
        const localVersions = findAllVersions(toolName2, arch);
    ...
    exports.find = find2;
    function findAllVersions(toolName2, arch) {
      const versions = [];
      arch = arch || os.arch();
      const toolPath = path2.join(_getCacheDirectory(), toolName2);
...
async function install(version2, chmod) {
  const extension = process.platform === "win32" ? ".exe" : "";
  const url = downloadURL(version2);
  const binaryPath = await download(version2, extension, url);
...
function downloadURL(version2) {
  ...
  const isHigherVersion = isVersionGreaterThan371(version2);
  ...
  if (isHigherVersion && extension !== "exe") {
    ...
    return `https://github.com/getsops/sops/releases/download/v${version2}/sops-v${version2}.${extension}.${arch}`;
  }
...
async function run() {
  try {
    ...
    const version2 = core4.getInput("version", required);
    ...
    const sopsPath = await install(version2, fs.chmodSync);
